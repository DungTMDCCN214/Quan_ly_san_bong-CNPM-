        ...
        <form method="POST" action="searchBooking" id="applyForm">
            ...
            <div class="court-group">
                ...
                <% for (BookingDetailView slot : courtSlots) {
                    ...
                    // Logic disable:
                    // Check-in  → enabled chỉ khi BOOKED; disabled khi IN PROGRESS/FINISHED/CANCELLED
                    // Check-out → enabled chỉ khi IN PROGRESS; disabled các trạng thái khác
                    // Hủy       → enabled chỉ khi BOOKED; disabled khi IN PROGRESS/FINISHED/CANCELLED
                    boolean canCheckin  = "BOOKED".equals(st);
                    boolean canCheckout = "IN PROGRESS".equals(st);
                ...
                <div class="slot-row" id="row_<%= did %>">
                    ...
                    <div class="slot-actions">
                        <%-- Check-in --%>
                        <button type="button"
                                class="btn-action btn-checkin <%= !canCheckin ? "disabled" : "" %> <%= "IN PROGRESS".equals(st)||"FINISHED".equals(st) ? "active" : "" %>"
                                id="btn_checkin_<%= did %>"
                                <%= !canCheckin ? "disabled" : "" %>
                                onclick="handleAction(<%= did %>, 'IN PROGRESS')">
                            ✅ Check-in
                        </button>

                        <%-- Check-out --%>
                        <button type="button"
                                class="btn-action btn-checkout <%= !canCheckout ? "disabled" : "" %> <%= "FINISHED".equals(st) ? "active" : "" %>"
                                id="btn_checkout_<%= did %>"
                                <%= !canCheckout ? "disabled" : "" %>
                                onclick="handleAction(<%= did %>, 'FINISHED')">
                            🚪 Check-out
                        </button>
...
<script>
    ...
    function refreshRow(did, st) {
        ...
        const badgeMap = {
            ...
            'IN PROGRESS': ['sb-playing',   'Đang chơi'],
            'FINISHED':    ['sb-done',      'Đã trả sân'],
        ...
        if (isPending) {
            ...
            let actionName = '';
            if (st === 'IN PROGRESS') actionName = 'Check-in';
            if (st === 'FINISHED') actionName = 'Check-out';
        ...
        } else if (effectiveSt === 'IN PROGRESS') {
            ci.classList.add('active');
            co.classList.remove('disabled'); co.disabled = false;
        } else if (effectiveSt === 'FINISHED') {
            ci.classList.add('active');
        ...
        if (isPending) {
            if (st === 'IN PROGRESS') { ci.classList.add('pending'); ci.innerHTML = '<i class="fas fa-hourglass-half"></i> Chờ Check-in'; }
            if (st === 'FINISHED')    { co.classList.add('pending'); co.innerHTML = '<i class="fas fa-hourglass-half"></i> Chờ Check-out'; }
